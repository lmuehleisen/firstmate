...
| Agy | Nudge | `.agents/hooks.json` invokes `bin/fm-agy-hook.sh` on `PreInvocation`; only `invocationNum=0` can inject the shared startup nudge, and only until this session owns the home lock. | Agy has no native SessionStart event; verified interactive first-invocation delivery does not establish a compaction or instruction-refresh channel. |
