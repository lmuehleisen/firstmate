...
Its `StopFailure` cases dispatch the tracked registrations by event, so an API-error turn end yields exactly one recovery and the next normal turn end runs only the `Stop` hooks.
They also cover the reset-time and backoff waits, a failed recovery waiting again, away-mode, halt, and live-continuity stand-downs, supersession by an ordinary `Stop` or a newer failure, and a turn already in progress.
