...
Claude kills the hook's process tree at its declared timeout and delivers nothing from a killed hook, so the hook passes the arm a deadline derived from that timeout, and a cycle with nothing to report closes before it with one queued no-op `check: autoarm-deadline` wake; the rewake turn acknowledges it, and its own Stop re-arms with a fresh timeout.
...
`tests/fm-claude-stop-autoarm.test.sh` covers the auto-arm's scope, stale and live session owners, unchanged AFK and need boundaries, single-flight, bounded failure retries, benign live-watcher cycle ends, one-notice failure episodes, an actionable wake that a leftover failure episode must not silence, exit-2 translation, and host-timeout HUP/TERM/INT translation into the same durable failure handoff.
It also derives the arm deadline from a fixture's declared hook timeout and runs the real arm and watcher under a 20-second declared timeout, which must close early with one queued no-op wake and a rewake; `tests/fm-watch-arm.test.sh` covers the same deadline for an arm attached to a watcher it does not own.
