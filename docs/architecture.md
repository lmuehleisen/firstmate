...
Task metadata identity is the single `pr=` URL matching the sidecar, so well-formed keys that later writers append are not part of the binding, while malformed content after `pr=` still refuses the poll, and recorded file identities match by inode on the state device so a reboot that renumbers the device id does not disarm a poll.
...
Its `--stop-failure` registration covers a turn that ends on an API error, which fires `StopFailure` instead of `Stop`: it waits out the usage limit or a bounded backoff, then starts one recovery turn whose `Stop` re-arms.
...
A digest too long to type without being folded as pasted content goes to a file under `state/.subsuper-digests/`, and the typed line only names that file (`escalate_flush` in `bin/fm-supervise-daemon.sh` owns the threshold).
...
A `https://github.com/<owner>/<repo>/pull/<n>` URL requires `gh` and `jq`, is merged only after one live read confirms the pull request is open, not a draft, mergeable, conflict-free, and every unwaived check is green at the current head, then `gh pr merge` binds that verified head with `--match-head-commit`, defaulting to `--squash` and preserving explicit merge-method flags.
...
A pool worktree is only returned after teardown passes the slot-ownership proof: a contradictory task record, a supported live endpoint, or a Treehouse lease held by anyone but this task refuses without touching either task, and no discard authority relaxes that.
