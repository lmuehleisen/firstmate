...
- `bin/fm-pr-merge.sh`: a merge the words call for proceeds only when the task's recorded yolo posture is on or its id is in the record's merge-grant list, which the captain names at entry and the words never supply, and the pull request is green at its live head, synchronously, under the record lock; any other merge holds for the return, whatever the words say.
