...
# fm_dod_block <no-mistakes|direct-PR|local-only> <task-id> [pipeline] prints the
...
# bin/fm-promote.sh refuse leftover placeholders and a `## Captain's intent`
...
fm_ship_rule_one() {  # <no-mistakes|direct-PR|local-only> <task-id> [pipeline]
  local mode=$1 id=$2 pipeline=${3:-}
...
fm_dod_block() {  # <mode> <task-id> [pipeline]
  local mode=$1 id=$2
  if [ "$mode" = no-mistakes ] && [ "${3:-}" = pipeline ]; then
    mode=no-mistakes-pipeline
  fi
  case "$mode" in
    direct-PR)
      cat <<EOF
