...
#     - <task-id>                one per line (the empty list is `merge_grants: -`);
#                                a record without the field grants nothing
...
# script: bin/fm-pr-merge.sh reads the record's merge grants as away merge
...
fm_afk_contract_render_readback() {  # <path> <title>
  local path=$1 title=$2 words expected spend grants grant_list id
  expected=$(fm_afk_contract_read_field "$path" expected_return)
  spend=$(fm_afk_contract_read_field "$path" spend_max_concurrent_workers)
...
# EXPECTED_RETURN, SPEND, MERGE_GRANTS, FM_AFK_CONTRACT_SCALARS_GIVEN).
