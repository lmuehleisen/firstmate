...
fm_backend_tmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # as muse: each live process name is a bare word - `omp` (verified, omp
    # 18.1.11) and `agy` (verified, agy 1.2.0) - and a glob would claim
    # unrelated commands such as ompd, comp, legacy, or agyrate. agy needs an
    # entry here rather than relying on the path fallback below because it
    # installs no per-task busy wiring: the backend classifier is the ONLY state
    # source for an agy task, so a missing name here makes every control verb
    # refuse on an 'ambiguous' endpoint.
