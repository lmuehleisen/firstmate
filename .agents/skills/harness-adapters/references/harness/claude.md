...
When Claude has never asked, it may leave both flags absent or persist both as false; registration leaves either form untouched, so an import dialog still renders if the worker's import chain reaches outside the project even though trust registers normally.
When the project entry instead records an explicit decline (`hasClaudeMdExternalIncludesWarningShown===true` and `hasClaudeMdExternalIncludesApproved===false`), the whole registration refuses - including the trust flag - rather than override the human's choice, so that spawn wedges on the trust dialog before it would even reach the import one.
An approved value of false with warning-shown false is Claude's untouched default, not a human decline, so trust registration proceeds while both import flags remain unchanged.
