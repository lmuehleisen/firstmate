...
1. **Write the record first, in this same turn.**
   ...
   The words are the whole mandate: `bin/fm-afk-contract.sh` records them exactly as given, with no clause fields, verbs, or ids, and by the captain's mandate no parser, tokenizer, classifier, or grammar reads them anywhere.
