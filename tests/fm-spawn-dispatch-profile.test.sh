...
test_claude_threads_model_and_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "claude --permission-mode auto --add-dir '$(cd "$HOME_DIR/state" && pwd -P)' --add-dir '$(cd "$HOME_DIR/data/$id" && pwd -P)' --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' $CLAUDE_CONTROL_CHANNEL_FLAG --model 'sonnet' --effort 'high'" \
...
test_claude_long_launch_is_delivered_intact() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  expected=$(fork_claude_expected_launch "$HOME_DIR" "$id")
  [ "${#expected}" -gt 1024 ] \
