...
SH
  ...
  # fm-devin-lib.sh, fm-agy-lib.sh, and fm-worker-tmux-lib.sh: teardown sources
  # the fork-only retires.
  ln -s "$ROOT/bin/fm-devin-lib.sh" "$fake/bin/fm-devin-lib.sh"
  ln -s "$ROOT/bin/fm-agy-lib.sh" "$fake/bin/fm-agy-lib.sh"
  ln -s "$ROOT/bin/fm-worker-tmux-lib.sh" "$fake/bin/fm-worker-tmux-lib.sh"
