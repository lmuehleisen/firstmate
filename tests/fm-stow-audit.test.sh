...
# unique fact is refused by name while a genuine consolidation and an archived
# removal pass without a false alarm, and the perishable referent check
# surfaces a resolved backlog id, leaves an open one alone, keeps other tiers
# out of scope, and reads an unresolvable referent as unknown rather than as
# either verdict.
set -u
...
(cd "$home" \
  ...
  && tasks-axi "done" fm-fixed-task-b2) >/dev/null 2>&1 \
