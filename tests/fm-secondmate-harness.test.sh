...
test_spawn_secondmate_harness_model_token() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --permission-mode auto --add-dir '$(cd "$w/home/state" && pwd -P)' --add-dir '$(cd "$sm/data" && pwd -P)' --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' --model 'opus'" \
...
test_spawn_secondmate_harness_model_and_effort_tokens() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --permission-mode auto --add-dir '$(cd "$w/home/state" && pwd -P)' --add-dir '$(cd "$sm/data" && pwd -P)' --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' --model 'opus' --effort 'high'" \
