...
config/crew-permissions  optional worker review posture for Claude, Codex, and agy launches: absent or "auto" uses automatic review, "manual" uses human review, and no value bypasses permissions; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Worker permission mode"
...
state/               runtime records and signals; gitignored
  ...
  <id>.agy-permission.json <id>.agy-permission-pending/ <id>.agy-permission-cache/  firstmate-owned per-task agy bypass permission policy file, pending-escalation markers, and approved-verdict cache read by bin/fm-agy-permission-policy.sh's hooks; exists only for --agy-bypass spawns; removed by teardown
  agy-permission-log.jsonl  home-wide append-only log of every agy worker tool call, written by bin/fm-agy-hook.sh's log-only observer, plus the bypass adapter's armed and decision lines; safe to delete
  ...
  .stow-audit/       stow-pass before-state snapshot of the startup-memory files plus that pass's relocation receipts; bin/fm-stow-audit.sh owns it (snapshot writes, verify diffs), and it is safe to delete between passes
  ...
  .afk-contract      the away-posture record: the captain's verbatim away words, expected return, reach profile, spend cap, and the merge grants named with --grant; written only by bin/fm-afk-contract.sh in the same turn as /afk, archived under afk-contracts/ at return; its presence IS the away posture in every harness; its sibling .afk-contract.lock serializes actions authorized by the live record (contract: bin/fm-afk-contract.sh)
  ...
  .claude-autoarm.lock .claude-autoarm-epoch .claude-autoarm-failure-notified .claude-autoarm-failure-alarmed .claude-stopfailure .turnend-claude-blocks .turnend-claude-blocks.lock   Claude Stop auto-arm single-flight, epoch, failure-episode, attended-alarm, StopFailure-recovery, guard-budget, and budget-lock records; never touch
...
When a scout's deliverable is a visual artifact the captain will iterate on and Lavish is available to it, keep it alive and follow the crew-hosted Lavish board contract in `docs/configuration.md` rather than arming or polling the board from firstmate.
